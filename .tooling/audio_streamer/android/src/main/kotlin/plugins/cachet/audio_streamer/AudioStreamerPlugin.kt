...
class AudioStreamerPlugin : FlutterPlugin, RequestPermissionsResultListener, EventChannel.StreamHandler, ActivityAware {
    ...
    // Flutter Engine 要求 EventSink 的回调在主线程执行。
    // 录音采集运行在独立线程里，若直接回调 error/success，会触发 @UiThread 崩溃。
    // 因此统一通过 dispatcher 收口线程切换，避免 success/error 逻辑再次分叉。
    private var eventSinkDispatcher: EventSinkDispatcher? = null
    ...
    override fun onListen(arguments: Any?, events: EventSink?) {
        this.eventSinkDispatcher = events?.let(::EventSinkDispatcher)
        sampleRate = (arguments as Map<*, *>)["sampleRate"] as Int
        if (sampleRate < 4000 || sampleRate > 48000) {
            eventSinkDispatcher?.error(
                "SampleRateError",
    ...
    private fun streamMicData() {
        ...
        try {
            ...
            if (audioRecord.state != AudioRecord.STATE_INITIALIZED) {
                ...
                eventSinkDispatcher?.error("MIC_ERROR", "Audio Record can't initialize!", null)
                recording = false
                return
            }
            ...
            while (recording) {
                ...
                if(readSize > 0) {
                    ...
                    eventSinkDispatcher?.success(audioBufferList)
                }
            }
        } catch (e: Exception) {
            Log.e(logTag, "Error while recording audio", e)
            eventSinkDispatcher?.error("MIC_ERROR", "Error while recording audio", e.message)
        } finally {
            if (::audioRecord.isInitialized) {
...
internal class EventSinkDispatcher(
    private val eventSink: EventSink,
    private val mainHandler: Handler = Handler(Looper.getMainLooper()),
) {
    // 这里不判断当前线程，而是始终投递到主线程，保持 success/error/endOfStream
    // 的行为一致，避免后续某条分支忘记切线程后再次引入同类崩溃。
