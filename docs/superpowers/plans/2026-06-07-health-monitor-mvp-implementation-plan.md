...
- [x] 为活动、姿势、位置、噪音、数字生活、提醒记录分别建表
- [x] 为聚合查询建立 repository
...
- [x] 通知常驻与状态展示
- [x] 异常恢复
> 说明：这一任务已推进到可用的 Android 原生系统边界，包含前台服务、通知通道、WorkManager 调度入口、异常恢复路径与可测的原生单测骨架，并已完成 `:app:testDebugUnitTest --tests "com.example.health_monitor.background.*"` 本地验证。
...
- [x] 数字生活替代指标后台刷新策略
- [x] 状态恢复与限制识别
> 说明：这一任务当前已补齐 iPhone 后台刷新策略、宿主状态模型、Flutter 桥接服务、诊断页展示与 iOS 原生桥接骨架。后续仍需补齐真实系统级后台刷新配置、权限联动与真机验证。
...
- Create: `C:\Users\13692\Documents\health_monitor\test\domain\background\background_capture_state_test.dart`
...
- 已补齐 iPhone 后台刷新策略、宿主状态模型、Flutter 桥接服务、调试页展示与 iOS 原生桥接骨架。
- 已补齐 Android 前台服务策略、原生调度门面、通知组装、Worker 骨架与恢复刷新入口，代码层与本地原生单测验证已经接通。
- 已补齐 Android 前台服务编排器、周期任务计划器、执行器和可恢复状态仓，并通过 `:app:testDebugUnitTest --tests "com.example.health_monitor.background.*"` 本地验证收口。
- 已补齐 Android 前台 `Service` 壳、WorkManager worker 壳、可持久化状态仓和请求编码器，系统级恢复链路已具备最小实现。
- 当前仍缺 iPhone 平台侧后台策略实现。
