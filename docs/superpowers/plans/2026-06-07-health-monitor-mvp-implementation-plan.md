...
> 2026-06-10 更新：
> 已补齐首页对真实 `OverviewViewModel` 的消费，移除核心静态占位文案；
> 已接入真实规则输出、权限不足分支、数据不足分支，并补充首页 Widget 测试验证真实文案渲染。
...
> 2026-06-10 更新：
> 简报页、我页、提醒原因页已接入真实 Provider 输出；
> 提醒记录页已接入真实规则生成的提醒记录与详情跳转，不再使用空的静态仓库；
> 已补齐提醒记录持久化历史链路：规则结果会去重写入提醒仓储，提醒记录页改为读取持久化历史，应用重进后仍可查看最近提醒历史。
> 已补齐我页权限交互中的特殊授权处理：`Usage Access` 走 Android 专用桥接与系统设置页，`backgroundCapture` 的后台权限引导也已补齐特殊权限声明与设置页入口。
