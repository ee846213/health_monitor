...
class MotionCaptureService {
  ...
  }) : _sensorStreamFactory = sensorStreamFactory ?? _defaultSensorStreamFactory;
    ...
    return _sensorStreamFactory(samplingPeriod: samplingPeriod)
        .transform<ActivitySample>(
          StreamTransformer<MotionVectorSample, ActivitySample>.fromHandlers(
            handleData: (MotionVectorSample sample, EventSink<ActivitySample> sink) {
              sink.add(_toActivitySample(sample));
            },
            handleError: (Object error, StackTrace stackTrace, EventSink<ActivitySample> sink) {
              sink.addError(
