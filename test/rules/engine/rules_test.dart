...
RuleInput _fullInput({
  ...
  List<AmbientLightSample> lights = const [],
...
void main() {
  ...
  group('PostureRule', () {
    ...
    test('行走占比高不再直接触发边走边看提醒', () {
      final samples = List<ActivitySample>.generate(
          6,
          (_) => ActivitySample(
              capturedAt: now,
              duration: ms(10),
              ...
              stepCount: 400,
              source: MotionSampleSource.sensorFusion));
      samples.addAll(List<ActivitySample>.generate(
          4,
          (_) => ActivitySample(
              capturedAt: now,
              duration: ms(10),
  ...
  group('UsageRule', () {
    ...
    test('夜间看屏超阈值应触发提醒', () {
      final input = _fullInput(usages: <DigitalUsageSummary>[
        DigitalUsageSummary(
            ...
            screenOnDuration: ms(300),
            unlockCount: 30,
            nighttimeUsageDuration: ms(80),
    ...
    test('查看频率判断应优先使用 viewCount 而不是 unlockCount', () {
      final input = _fullInput(usages: <DigitalUsageSummary>[
        DigitalUsageSummary(
          date: DateTime(2026, 6, 10),
          screenOnDuration: ms(90),
      ...
      final v = rule.evaluate(input);
      expect(v.where((x) => x.summary == '查看频率偏高'), isEmpty);
    });
  });
  ...
  group('EnvironmentRule', () {
    ...
    test('高噪音应触发提醒', () {
      final input = _fullInput(noises: <NoiseSample>[
        NoiseSample(
            capturedAt: DateTime(2026, 6, 10, 23),
    ...
    test('白天长时间过暗应产出 warning', () {
      final input = _fullInput(lights: <AmbientLightSample>[
        AmbientLightSample.fromLux(
          capturedAt: DateTime(2026, 6, 10, 11),
          duration: const Duration(minutes: 70),
          lux: 6,
        ),
      ]);
      final v = rule.evaluate(input);
      expect(v.where((x) => x.summary == '白天光线偏暗'), isNotEmpty);
    });
  ...
  group('ReminderRule', () {
    test('应汇总所有触发提醒的规则结论并去重', () {
      final input = _fullInput(
        activities: List<ActivitySample>.generate(
            4,
            (_) => ActivitySample(
                capturedAt: now,
                duration: ms(60),
        ...
        metrics: <DailyMetrics>[
          DailyMetrics(
            ...
            postureRiskCount: 2,
            highNoiseExposureDuration: ms(35),
          ),
        ],
      );
      ...
      expect(reminders.where((x) => x.reminderType == 'sedentaryBreak'),
          isNotEmpty);
      expect(
          reminders.where((x) => x.reminderType == 'postureRisk'), isNotEmpty);
      expect(reminders.where((x) => x.reminderType == 'noisyEnvironment'),
          isNotEmpty);
      // 去重测试: sedentaryBreak 应只出现一次
      expect(
          reminders.where((x) => x.reminderType == 'sedentaryBreak').length, 1);
    });
  });
}
